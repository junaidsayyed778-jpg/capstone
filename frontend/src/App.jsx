...
const timeoutId = setTimeout(() => controller.abort(), 120000);
      ...
      setTerminalLogs((prev) => [
        ...prev,
        `Failed to reach API endpoint at ${API_BASE_URL || "/api"}.`,
      ]);
  ...
  const sendMessage = async (content) => {
    ...
    try {
      ...
      setMessages((prev) => [
        ...prev,
        { role: "ai", content: data.reply || "Changes applied." },
      ]);

      // Add a system log outputting what changing
      setTerminalLogs((prev) => [
        ...prev,
        `[AI Execution]: Generating code based on prompt "${content.substring(0, 20)}..."`,
      ]);
    } catch (err) {
      ...
      setTimeout(() => {
        setMessages((prev) => [
          ...prev,
          {
            role: "ai",
            content:
              "I couldn't reach the backend server, but if I could, I would be generating that code right now.",
          },
        ]);
        setTerminalLogs((prev) => [
          ...prev,
